...
export default function MovieWishlist() {
  const [wishlist, setWishlist] = useState<WishlistMovie[]>(() => {
    ...
    if (saved) {
      try {
        ...
        console.error('Gagal memuat film wishlist', e);
        return [];
      }
    }
    // Pre-populate some aesthetic retro wishlist items
    return [
      {
        id: 'mw-1',
        title: 'Spirited Away: Sequel Movie',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      // Add
      const newItem: WishlistMovie = {
        id: `m-wish-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`,
        title: title.trim(),
        genre: genre.trim(),
        addedAt: Date.now()
      };
  ...
  const handleEdit = (item: WishlistMovie) => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-[#fdfaf2] dark:bg-[#2d2820] p-4 rounded-[4px] border border-[#d4c9a8] dark:border-[#4b463e] shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-display font-bold text-[#3d3527] dark:text-[#e8dcc4] uppercase tracking-wider">
            Sistem Arsip Wishlist Film ({wishlist.length})
          </span>
        </div>
        <button
          id="add-movie-wishlist-btn"
          onClick={() => {
            if (isFormOpen) {
              resetForm();
            } else {
            ...
            <form onSubmit={handleSubmit} className="bg-[#fdfaf2] dark:bg-[#2d2820] p-6 rounded-[4px] border border-[#d4c9a8] dark:border-[#4b463e] shadow-tactile space-y-4">
              <div className="flex justify-between items-center border-b border-[#d4c9a8]/40 dark:border-[#4b463e]/40 pb-3">
                <h3 className="font-display font-bold text-[#3d3527] dark:text-[#e8dcc4] text-xs uppercase tracking-wider flex items-center gap-2">
                  <Clapperboard className="w-3.5 h-3.5 text-[#a23b2c] dark:text-[#ff816c]" />
                  {editingId ? 'Edit Catatan Wishlist Film' : 'Registrasi Wishlist Film Baru'}
                </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-display font-bold text-stone-500 dark:text-stone-400 uppercase tracking-wider">Judul Film *</label>
                  <input
                    type="text"
                    required
      ...
      <div className="bg-[#fdfaf2] dark:bg-[#2d2820] p-4 rounded-[4px] border border-[#d4c9a8] dark:border-[#4b463e] shadow-sm flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
        ...
        <div className="relative w-full md:w-64">
          ...
          <input
            type="text"
            placeholder="Cari film wishlist..."
            value={search}
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <div className="flex items-center gap-1.5">
            ...
            >
              ...
              <option value="title">Judul Film</option>
            </select>
            <button
      ...
      {processedWishlist.length === 0 ? (
        <div className="bg-[#fdfaf2] dark:bg-[#2d2820] p-12 text-center rounded-[4px] border border-dashed border-[#d4c9a8] dark:border-[#4b463e] shadow-sm">
          <Clapperboard className="w-12 h-12 text-stone-300 dark:text-stone-600 mx-auto mb-4" />
          <h4 className="text-sm font-display font-bold text-stone-600 dark:text-stone-300 uppercase tracking-wider">Tidak ada data wishlist film</h4>
          <p className="text-xs text-stone-400 dark:text-stone-500 mt-2 max-w-sm mx-auto leading-relaxed">
            Mulailah menulis film-film impian yang ingin Anda simpan dengan menekan tombol Tambah Wishlist di atas.
          </p>
        </div>
      ) : (
        <div className="space-y-6">
